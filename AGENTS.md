...
- `PythonEnvironment.svelte` is the embeddable component (the Python counterpart to `KarelEnvironment`), configured by a `PythonConfig`. Python is **unrestricted** by default; the opt-in `allowedFeatures` allowlist is deliberately deferred — see `PROGRESS.md`.
- **Breakpoints are set by clicking a line number** and travel to the worker through a **third shared buffer**, not `postMessage` — they can be toggled while the program is paused, and a paused worker is blocked in `Atomics.wait`. `breakpoints.ts` holds which lines may carry one and the toggle/persist arithmetic; the editor holds the authoritative set as CodeMirror document positions so a mark rides along with its statement.
- `controls.ts` is the control row's state machine (which buttons are live for each `RunnerStatus`), kept out of the component so every state is assertable under Vitest. `PythonInterpreterDesign.md` §12 is the full spec for the controls, layout, breakpoints, and the post-mortem "final snapshot" banners.
- **Python exercises are auto-graded on captured stdout.** `src/lib/curriculum/exercise-fixtures.ts` holds a verified solution, the queued `input()` answers and the exact captured stdout for every authored Python exercise, and `testsFor(key)` turns that into the `PythonConfig.tests` each lesson wires up — so the transcript a lesson prints, the contract its prose states, and what "Run tests" checks are one array of strings. **Edit a lesson's sample run and its fixture together**; `exercise-fixtures.test.ts` fails if they stop agreeing or if a fixture is never wired up, and `tests/python.test.ts` executes all fifteen solutions in a browser. Comparison ignores trailing spaces but not blank lines, and prompt wording counts — `PythonInterpreterDesign.md` §13.2 for why.
- `exercise-tests.ts` is the test harness, kept apart from `PythonRunner` behind a four-method `TestHost` so every branch (dry answer queue, leftover answers, a program that raises or never ends) is assertable under Vitest against a scripted fake worker.
- **`PythonConfig.showVisualizer` is the _initial_ state only.** The student can show or hide the panel on any exercise, and that choice persists under the same `persistenceKey` as their code and breakpoints.
